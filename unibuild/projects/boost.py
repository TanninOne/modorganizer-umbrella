...
Project("boost") \
	.depend(patch.Copy("stage/lib/boost_python-vc{}-mt-{}.dll".format(config['vc_version'].replace(".",""),boost_version.replace(".","_")[:-2]),os.path.join(config['__build_base_path'], "install", "bin", "dlls"))
		.depend(b2.B2(name="Shared").arguments(["address-model={}".format("64" if config['architecture'] == 'x86_64' else "32"),
												...
												"toolset=msvc-{}".format(config['vc_version']),
												"link=shared",
